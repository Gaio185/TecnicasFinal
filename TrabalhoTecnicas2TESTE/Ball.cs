...
namespace TrabalhoTécnicas2
{
    public class Ball
    {
        ...
        public Ball(Game1 game1, Player targetPlayer, Vector2 positionX)
        {
            Random random = new Random();
            position = positionX;
            player = targetPlayer;
            speed = 150f; // Velocidade da ball
            damage = 0.03f; // Dano causado pela a ball
            game = game1;
            animationFrames = new List<Texture2D>();
            // Adicione as texturas dos quadros da animação à lista
            animationFrames.Add(game.Content.Load<Texture2D>("Ball"));
            animationFrames.Add(game.Content.Load<Texture2D>("Ball2"));
            animationFrames.Add(game.Content.Load<Texture2D>("Ball3"));
            animationFrames.Add(game.Content.Load<Texture2D>("Ball4"));
            currentFrame = 0;
